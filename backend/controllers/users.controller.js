...
const banUserById = async (req, res, next) => {
	try {
		const id = req.params.id;
		const options = { password: 0 };
		let user = await findWithId(User, id, options);
...
const banAllUsers = async (req, res, next) => {
	try {
		await User.updateMany({}, { isBanned: false });

		return successResponse(res, {
			statusCode: 200,
			message: 'All users banned successfully',
		});
	} catch (error) {
		next(error);
	}
...
module.exports = { getUsers, getUserById, deleteUserById, banUserById, banAllUsers, updateUser };
